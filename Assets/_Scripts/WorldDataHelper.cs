...
{
    public static Vector3Int ChunkPositionFromBlockCoords(World world, Vector3Int position) // this converts the player position into a chunk position
    {
    ...
    internal static List<Vector3Int> GetChunkPositionsAroundPlayer(World world, Vector3Int playerPosition)
    {
        ...
        for (int x = startX; x <= endX; x += world.chunkSize) // refactor this into a separate method later (Used in GetPositionsAroundPlayer and GetDataPositionsAroundPlayer)
        {
    ...
    internal static List<Vector3Int> GetDataPositionsAroundPlayer(World world, Vector3Int playerPosition)
    {
        ...
        for (int x = startX; x <= endX; x += world.chunkSize) // refactor this into a separate method later
        {
            for (int z = startZ; z <= endZ; z += world.chunkSize)
            {
                Vector3Int chunkPos = ChunkPositionFromBlockCoords(world, new Vector3Int(x, 0, z));
    ...
    internal static List<Vector3Int> SelectPositionsToCreate(WorldData worldData, List<Vector3Int> allChunkPositionsNeeded, Vector3Int playerPosition)
    {
        return allChunkPositionsNeeded
            ...
            .OrderBy(pos => Vector3.Distance(playerPosition, pos)) // Sort by square magnitude for higher performance
            .ToList();
    }
